...
static inline HI_VOID HIFB_PROC_PrintVoCallBackInfo(HIFB_PAR_S *pstPar, struct seq_file *pstSeqFile);
...
HI_VOID HIFB_PROC_CountVoCallBackScanLineAbnormalTimes(HI_U32 LayerID)
{
    ...
    if (pstPar->stProcInfo.stCheckInterruptState.VoCallBackInScanLine >= pstPar->stProcInfo.stCheckInterruptState.VoCallBackStartScanLine)
    {
	if (pstPar->stProcInfo.stCheckInterruptState.VoCallBackOutScanLine > pstPar->stProcInfo.stCheckInterruptState.VoCallBackInScanLine)
	{/** OK **/
	    return;
	}

	if (pstPar->stProcInfo.stCheckInterruptState.VoCallBackOutScanLine < pstPar->stProcInfo.stCheckInterruptState.VoCallBackUpScanLine)
	{/** OK **/
	    return;
	}

	/** abnormal **/
	pstPar->stProcInfo.stCheckInterruptState.VoCallBackScanLineAbnormalNum++;
    }
    else if (pstPar->stProcInfo.stCheckInterruptState.VoCallBackInScanLine < pstPar->stProcInfo.stCheckInterruptState.VoCallBackUpScanLine)
    {
       if (pstPar->stProcInfo.stCheckInterruptState.VoCallBackOutScanLine < pstPar->stProcInfo.stCheckInterruptState.VoCallBackUpScanLine)
       {/** OK **/
	   return;
...
#if 0
       HI_PRINT("\n=============================================================\n");
       HI_PRINT("in %d out %d start %d up %d\n",pstPar->stProcInfo.stCheckInterruptState.VoCallBackInScanLine,
						pstPar->stProcInfo.stCheckInterruptState.VoCallBackOutScanLine,
						pstPar->stProcInfo.stCheckInterruptState.VoCallBackStartScanLine,
						pstPar->stProcInfo.stCheckInterruptState.VoCallBackUpScanLine);
       HI_PRINT("=============================================================\n");
#endif
       pstPar->stProcInfo.stCheckInterruptState.VoCallBackScanLineAbnormalNum++;
    }
...
HI_VOID HIFB_PROC_SetStartAndUpScanLine(HI_U32 StartScanLine, HI_U32 UpScanLine)
{
    ...
    info = s_stLayer[0].pstInfo;
    HIFB_CHECK_NULLPOINTER_UNRETURN(info);

    pstPar = (HIFB_PAR_S *)info->par;
    HIFB_CHECK_NULLPOINTER_UNRETURN(pstPar);

    pstPar->stProcInfo.stCheckInterruptState.VoCallBackStartScanLine = StartScanLine;
    pstPar->stProcInfo.stCheckInterruptState.VoCallBackUpScanLine = UpScanLine;
...
HI_VOID HIFB_PROC_SetVoCallBackInOutScanLine(HI_U32 LayerId, HI_U32 ScanLine, HI_BOOL bInScanLine)
{
    ...
    if (HI_TRUE == bInScanLine)
    {
       pstPar->stProcInfo.stCheckInterruptState.VoCallBackInScanLine = ScanLine;
    }
    else
    {
       pstPar->stProcInfo.stCheckInterruptState.VoCallBackOutScanLine = ScanLine;
    }
...
HI_VOID HIFB_PROC_VoCallBackCostTime(HI_U32 LayerId, HI_U32 TimeMs)
{
    ...
    pstPar->stProcInfo.stCheckInterruptState.VoCallBackCostTime[pstPar->stProcInfo.stCheckInterruptState.CountCurTimesForCostTime] = TimeMs;

    pstPar->stProcInfo.stCheckInterruptState.CountCurTimesForCostTime++;

    if (pstPar->stProcInfo.stCheckInterruptState.CountCurTimesForCostTime >= HIFB_PROC_VOCALLBACK_COUNT_COSTTIME)
    {
       pstPar->stProcInfo.stCheckInterruptState.CountCurTimesForCostTime = 0;
    }

    /** need min time is 1000 / 60fps**/
    if (TimeMs > 16)
    {
       pstPar->stProcInfo.stCheckInterruptState.CountVocallBackTimeAbnormalNum++;
    }
...
static HI_S32 HIFB_PrintHDProc(struct fb_info * info, struct seq_file *p, HI_VOID *v)
{
    ...
    HIFB_PROC_PrintVoCallBackInfo(par, p);
...
{
    HI_U8 Cnt = 0;
    PROC_PRINT(pstSeqFile,  "-------------------------- vo callback info ----------------------------\n");
    /** vo callback cost times **/
    /** 90%�жϺ���ʱ����Ϣ **/
    PROC_PRINT(pstSeqFile,  "current %d frame vocallback cost times with ms as follows\n",HIFB_PROC_VOCALLBACK_COUNT_COSTTIME);
    for (Cnt = 0; Cnt < HIFB_PROC_VOCALLBACK_COUNT_COSTTIME; Cnt++)
    {
	PROC_PRINT(pstSeqFile,	"[%d] ",pstPar->stProcInfo.stCheckInterruptState.VoCallBackCostTime[Cnt]);
    }
    PROC_PRINT(pstSeqFile,  "\nvo callback cost time much to 16ms times	  \t : %d\n", pstPar->stProcInfo.stCheckInterruptState.CountVocallBackTimeAbnormalNum);

    /** vo callback scanline informtion **/
    /** 90%�ж�ɨ������Ϣ **/
    PROC_PRINT(pstSeqFile,  "vo callback interrupt abnormal times	\t : %d\n", pstPar->stProcInfo.stCheckInterruptState.VoCallBackScanLineAbnormalNum);
    return;
}
...
static inline HI_VOID HIFB_PrintHwcRefreshStatus(struct fb_info *pstFbInfo, struct seq_file *pstSeqFile)
{
    ...
    DispBufStride = CONIFG_HIFB_GetMaxStride(pstPar->stExtendInfo.u32DisplayWidth,pstFbInfo->var.bits_per_pixel,&CmpStride,CONFIG_HIFB_STRIDE_16ALIGN);
    DispBufSize = HI_HIFB_GetMemSize(DispBufStride,pstPar->stExtendInfo.u32DisplayHeight);

#if !defined(CONFIG_HIFB_CALCSTRIDE_WITH_ONEPOINTTHREE)
